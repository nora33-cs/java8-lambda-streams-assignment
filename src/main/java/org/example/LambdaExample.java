...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        // 1- With functional interfaces declared
...
repository.select(ageGreaterThan5, sortByName);

        banner("Listing users with age > 5 sorted by name - lambda");
        // 2- With functional interfaces used directly
        repository.select(
    user -> user.age > 5,
    (u1, u2) -> u1.name.compareTo(u2.name)
        ...
        // 3- With functional interfaces declared
...
repository.select(ageLessThan10, sortByAge);
        ...
        // 4- With functional interfaces used directly
        repository.select(
    user -> user.age < 10,
    (u1, u2) -> Integer.compare(u1.age,u2.age)
        ...
        // 5- With functional interfaces declared
        Predicate<User> activeUser = new Predicate<User>() {
    @Override
    public boolean test(User user) {
        return user.active;
    }
...
repository.select(activeUser, sortbyname);

        banner("Listing active users sorted by name - lambda");
        // 6- With functional interfaces used directly
       repository.select(
    user -> user.active,
    (u1, u2) -> u1.name.compareTo(u2.name)
);
...
// 7- With functional interfaces declared
...
repository.select(activeuser, sortbyName);
        ...
        // 8- With functional interfaces used directly
        repository.select(
